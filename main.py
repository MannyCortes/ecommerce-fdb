...
async def main(file: UploadFile = File(...)):
    ...
    filename = f"raw_uploads/{uuid.uuid4()}.csv"
    try:
        ...
        async with session.client('s3', aws_access_key_id=aws_key_id, aws_secret_access_key=aws_secret_key) as s3:
                ...
                #TCP Backpressure
                await s3.upload_fileobj(file.file, os.getenv("S3_BUCKET_NAME"), filename, ExtraArgs={'ChecksumAlgorithm': 'SHA256'})
        ...
        file_location = f"s3://my-bucket/{filename}"
        message_payload = json.dumps({
        "task": "transform_csv",
        "originall_filename": file.filename,
        "file_location": f"s3://my-bucket/{filename}"
        })
        #lpsuh = lpush into  redis queue, FIFO/First In First Out 
        await redis_client.lpush("task_queue", message_payload)
        logging.info(f"File {filename} uploaded to S3 and task queued in Redis.")
        #redis automatically handles return responses
        #redis is asynchronou it also helps seperate fast api from pandas
        return Response(content=f"File {filename} uploaded to S3 and task queued in Redis.", status_code=status.HTTP_200_OK)
    except redis.RedisError as e:
        logging.error(f"Redis connection error: {e}")
        async with session.client('s3', aws_access_key_id=aws_key_id, aws_secret_access_key=aws_secret_key) as s3_idempotent:
            await s3_idempotent.delete_object(os.getenv("S3_BUCKET_NAME"), filename)
        ...
        return Response(content=f"Error Uploading file:{filename} - {e}", status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)
